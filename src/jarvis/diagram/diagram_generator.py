...
def case_decomposition_diagram(**kwargs):
    ...
    if len(function_list) > 0:
        _, consumer_function_list, producer_function_list = \
            util.get_cons_prod_from_view_allocated_flow(kwargs[XML_DICT_KEY_0_DATA_LIST],
                                                        kwargs[XML_DICT_KEY_13_VIEW_LIST],
                                                        kwargs[XML_DICT_KEY_15_FUN_CONS_LIST],
                                                        kwargs[XML_DICT_KEY_16_FUN_PROD_LIST],
                                                        function_list)
    ...
    activity_list = util.get_object_list_from_view(diagram_object_str,
                                                   kwargs[XML_DICT_KEY_10_ACTIVITY_LIST],
                                                   kwargs[XML_DICT_KEY_13_VIEW_LIST])

    if len(activity_list) > 0:
        _, consumer_activity_list, producer_activity_list = \
            util.get_cons_prod_from_view_allocated_flow(kwargs[XML_DICT_KEY_11_INFORMATION_LIST],
                                                        kwargs[XML_DICT_KEY_13_VIEW_LIST],
                                                        kwargs[XML_DICT_KEY_17_ACT_CONS_LIST],
                                                        kwargs[XML_DICT_KEY_18_ACT_PROD_LIST],
                                                        activity_list)
    ...
    query_inheritance.query_inheritance_remove_inherited_view(kwargs[XML_DICT_KEY_13_VIEW_LIST], v_inheritance)
...
def case_chain_diagram(**kwargs):
    ...
    if len(object_list_str) > 0:
        xml_activity_name_list = query_object.query_object_name_in_list(kwargs[XML_DICT_KEY_10_ACTIVITY_LIST])
        ...
        if result_activity:
            ...
            _, consumer_list, producer_list = \
                util.get_cons_prod_from_view_allocated_flow(kwargs[XML_DICT_KEY_11_INFORMATION_LIST],
                                                            kwargs[XML_DICT_KEY_13_VIEW_LIST],
                                                            kwargs[
                                                                XML_DICT_KEY_17_ACT_CONS_LIST],
                                                            kwargs[
                                                                XML_DICT_KEY_18_ACT_PROD_LIST],
                                                            activity_list)
            ...
            if len(function_list) > 0:
                ...
                _, consumer_list, producer_list = \
                    util.get_cons_prod_from_view_allocated_flow(kwargs[XML_DICT_KEY_0_DATA_LIST],
                                                                kwargs[XML_DICT_KEY_13_VIEW_LIST],
                                                                kwargs[
                                                                    XML_DICT_KEY_15_FUN_CONS_LIST],
                                                                kwargs[
                                                                    XML_DICT_KEY_16_FUN_PROD_LIST],
                                                                function_list)
        ...
        if len(object_list_str) == 1 and \
            ...
            if all(i in query_object.query_object_name_in_list(kwargs[XML_DICT_KEY_10_ACTIVITY_LIST])
                ...
                if len(xml_information_list) > 0:
                    if len(xml_information_list) != len(kwargs[XML_DICT_KEY_11_INFORMATION_LIST]):
                        xml_cons = [i for i in kwargs[XML_DICT_KEY_17_ACT_CONS_LIST]
                                    if any(a == i[0] for a in [d for d in xml_information_list])]
                        xml_prod = [i for i in kwargs[XML_DICT_KEY_18_ACT_PROD_LIST]
                                    if any(a == i[0] for a in [d for d in xml_information_list])]
                    else:
                        xml_cons = kwargs[XML_DICT_KEY_17_ACT_CONS_LIST]
                        xml_prod = kwargs[XML_DICT_KEY_18_ACT_PROD_LIST]
                ...
                if len(xml_data_list) > 0:
                    ...
                    if len(xml_data_list) != len(kwargs[XML_DICT_KEY_0_DATA_LIST]):
                        ...
                        xml_prod = [i for i in kwargs[XML_DICT_KEY_16_FUN_PROD_LIST]
                                    if any(a == i[0] for a in [d for d in xml_data_list])]
                    else:
                        xml_cons = kwargs[XML_DICT_KEY_15_FUN_CONS_LIST]
                        xml_prod = kwargs[XML_DICT_KEY_16_FUN_PROD_LIST]
                ...
                if len(xml_data_list) > 0:
                    if len(xml_data_list) != len(kwargs[XML_DICT_KEY_0_DATA_LIST]):
                        kwargs[XML_DICT_KEY_15_FUN_CONS_LIST] = \
                            [i for i in kwargs[XML_DICT_KEY_15_FUN_CONS_LIST]
                             if any(a == i[0] for a in [d for d in xml_data_list])]
                        kwargs[XML_DICT_KEY_16_FUN_PROD_LIST] = \
                            [i for i in kwargs[XML_DICT_KEY_16_FUN_PROD_LIST]
                             if any(a == i[0] for a in [d for d in xml_data_list])]
                ...
                if len(xml_information_list) > 0:
                    if len(xml_information_list) != len(kwargs[XML_DICT_KEY_11_INFORMATION_LIST]):
                        kwargs[XML_DICT_KEY_17_ACT_CONS_LIST] = \
                            [i for i in kwargs[XML_DICT_KEY_17_ACT_CONS_LIST]
                             if any(a == i[0] for a in [d for d in xml_information_list])]
                        kwargs[XML_DICT_KEY_18_ACT_PROD_LIST] = \
                            [i for i in kwargs[XML_DICT_KEY_18_ACT_PROD_LIST]
                             if any(a == i[0] for a in [d for d in xml_information_list])]
                    # Else do nothing

                    kwargs[XML_DICT_KEY_11_INFORMATION_LIST] = xml_information_list
            ...
            for fun in temp_fun_set:
                new_consumer_list.extend(
                    get_main_elem_for_cons_prod_lists(
                        fun_elem, fun, kwargs[XML_DICT_KEY_15_FUN_CONS_LIST]
                    )
                )
                new_producer_list.extend(
                    get_main_elem_for_cons_prod_lists(
                        fun_elem, fun, kwargs[XML_DICT_KEY_16_FUN_PROD_LIST]
                    )
                )
    ...
    for phy_elem in new_phy_elem_list:
        ...
        if temp_activity_set is not None:
            for activity in temp_activity_set:
                new_consumer_list.extend(
                    get_main_elem_for_cons_prod_lists(
                        phy_elem, activity, kwargs[XML_DICT_KEY_17_ACT_CONS_LIST]
                    )
                )
                new_producer_list.extend(
                    get_main_elem_for_cons_prod_lists(
                        phy_elem, activity, kwargs[XML_DICT_KEY_18_ACT_PROD_LIST]
                    )
                )
